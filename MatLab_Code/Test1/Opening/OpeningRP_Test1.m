clear;clc;
files=dir('RP_O_3_*.csv');
if isempty(files)
    fprintf('[!] No RP_O_3_*.csv files found.\n');return;
end
% Numerical sort (0% -> 100%)
increments=zeros(length(files),1);
for i=1:length(files)
    num=regexp(files(i).name,'\d+','match');
    increments(i)=str2double(num{end});
end
[~,sort_idx]=sort(increments,'ascend');
files=files(sort_idx);
fprintf('   STEP 2: R&P OPENING (INCREMENTS 0%% -> 100%%)\n');
fprintf('%-15s | %-15s | %-15s\n','FILE','OPENING','STATUS');
fprintf('-------------------------------------------------------\n');
for i=1:length(files)
    nom_f=files(i).name;
    ...
    for f=1:size(data,1)
        ...
        if ~isempty(pts)
            ...
            if size(pts,1)<6
                ...
                % If gap < 12mm, consider full contact
